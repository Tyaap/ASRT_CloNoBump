...
public static class MemoryHelper
{
    ...
    public static string ReadString(int address, int length = -1)
    {
        return ReadString((uint)address, length);
    }
    public static string ReadString(uint address, int length = -1)
    {
        return ReadString((UIntPtr)address, length);
    }
    public static string ReadString(IntPtr address, int length = -1)
    {
        return ReadString((uint)address, length);
    }
    public static string ReadString(UIntPtr address, int length = -1)
    {
        if (length == -1)
        {
            length = 0;
    ...
    public static void Write(int address, string value, int bufferSize = 0)
    {
        Write(address, GetANSIstr(value, bufferSize));
    }
    public static void Write(uint address, string value, int bufferSize = 0)
    {
        Write(address, GetANSIstr(value, bufferSize));
    }
    public static void Write(IntPtr address, string value, int bufferSize = 0)
    {
        Write(address, GetANSIstr(value, bufferSize));
    }
    public static void Write(UIntPtr address, string value, int bufferSize = 0)
    {
        Write(address, GetANSIstr(value, bufferSize));
    }

    public static byte[] GetANSIstr(string value, int bufferSize = 0)
    {
        byte[] byteStr = Encoding.UTF8.GetBytes(value + (char)0);
        if (bufferSize == 0 || byteStr.Length <= bufferSize)
        {
            return byteStr;
        }
        else
        {
            byte[] byteStrTrunk = new byte[bufferSize];
            Array.Copy(byteStr, byteStrTrunk, bufferSize - 1);
            return byteStrTrunk;
        }
    }
